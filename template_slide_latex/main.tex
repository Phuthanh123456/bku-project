...
% nội dung bị mang đi dựng lại ở chỗ khác. Lệnh mềm (\newcommand) đặt vào đó sẽ
% bung ra sai lúc dựng trang, và lỗi báo ở tận dòng \end{frame} chứ không báo ở
% dòng gây ra, nên rất khó truy. Đây chính là lỗi dòng 598 của slide 11:
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.46\textwidth}
      ...
      \begin{beamercolorbox}[wd=\textwidth, rounded=true, sep=6pt]{block body alerted}
        \textcolor{MLIOTink}{\footnotesize\textbf{Nguyên tắc xuyên suốt:} mọi lựa chọn
...
\begin{frame}[shrink=10]{Mục tiêu}
  \begin{columns}[T]
    ...
    \begin{column}{0.49\textwidth}
      ...
      \end{block}
      \begin{beamercolorbox}[wd=\textwidth, rounded=true, sep=6pt]{block body alerted}
        \textcolor{MLIOTink}{\footnotesize Mức đánh giá BLEU trên tst2013:
...
\begin{frame}[shrink=10]{Xử lý dữ liệu (1/2) — Làm sạch và chống rò rỉ}
  \begin{columns}[T]
    \begin{column}{0.46\textwidth}
      ...
      \end{block}
      \begin{beamercolorbox}[wd=\textwidth, rounded=true, sep=6pt]{block body alerted}
        \textcolor{MLIOTink}{\footnotesize\textbf{Chống rò rỉ:} 12 cặp câu của tst2012
  ...
  \begin{columns}[T]
    \begin{column}{0.49\textwidth}
      \begin{block}{Luồng dữ liệu}
        \scriptsize
        \texttt{src\_ids} $(B, S)$ $\rightarrow$ Embedding chung \\
        $\rightarrow$ \textbf{6 $\times$ EncoderLayer} \\
        \hspace{1.2em}self-attention (\textbf{có RoPE}) \\
        \hspace{1.2em}feed-forward SwiGLU \\
        $\rightarrow$ bộ nhớ encoder $(B, S, 512)$ \\[0.4em]
        \texttt{tgt\_ids} $(B, T)$ $\rightarrow$ Embedding chung \\
        $\rightarrow$ \textbf{6 $\times$ DecoderLayer} \\
        \hspace{1.2em}self-attention (\textbf{có RoPE}) \\
        \hspace{1.2em}cross-attention (\textbf{KHÔNG RoPE}) \\
        \hspace{1.2em}feed-forward SwiGLU \\
        $\rightarrow$ \textbf{RMSNorm cuối} \\
        $\rightarrow$ Linear chia sẻ trọng số \\
        $\rightarrow$ logits $(B, T, 32000)$
      \end{block}
    \end{column}
    \begin{column}{0.48\textwidth}
...
\begin{frame}[shrink=10]{Kiến trúc (2/3) — Bốn khối hiện đại, mỗi khối hai phương án}
  \begin{columns}[T]
    ...
    \begin{column}{0.46\textwidth}
      ...
      \end{block}
      \begin{beamercolorbox}[wd=\textwidth, rounded=true, sep=6pt]{block body alerted}
        \textcolor{MLIOTink}{\footnotesize Warmup và Label Smoothing \textbf{mặc định
...
% ------------------------------------------------------------
%  SLIDE 10 – Kiến trúc (3/3)
% ------------------------------------------------------------
\begin{frame}[shrink=10]{Kiến trúc (3/3) — Những chỗ sai mà chương trình không báo lỗi}
  \begin{columns}[T]
    \begin{column}{0.47\textwidth}
      \begin{block}{Ba điểm thiết kế bắt buộc nhớ}
        \begin{enumerate}
          \setlength{\itemsep}{0.25em}
          \item \textbf{RoPE chỉ áp cho self-attention.} Ở cross-attention, query nằm ở
            câu Việt còn key nằm ở câu Anh — hai chuỗi khác nhau nên "khoảng cách"
            giữa chúng vô nghĩa.
          \item \textbf{Pre-Norm là} $x + \mathrm{SubLayer}(\mathrm{Norm}(x))$,
            \textbf{không phải} $\mathrm{Norm}(x + \mathrm{SubLayer}(x))$.
          \item \textbf{Phải có một RMSNorm cuối} trước lớp Linear xuất từ vựng. Đường
            residual không bao giờ được chuẩn hóa nên giá trị phình dần; quên bước này
            không báo lỗi, chỉ làm mô hình mất ổn định.
        \end{enumerate}
      \end{block}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{block}{Bốn cái bẫy fp16 trên GPU T4}
        \scriptsize\bangchat
        \begin{tabular}{@{}p{1.9cm}p{1.7cm}p{2.2cm}@{}}
          \toprule
          \textbf{Chỗ} & \textbf{Phải làm} & \textbf{Nếu sai} \\
          \midrule
          Che mask trong attention &
          \texttt{finfo(dtype)} \texttt{.min} &
          \texttt{-1e9} thành $-\infty$, hàng bị mask ra \textbf{NaN} \\
          \addlinespace
          Bảng góc quay RoPE &
          tính ở float32 rồi ép kiểu &
          mất độ phân giải, \textbf{RoPE hỏng âm thầm} \\
          \addlinespace
          Bình phương trong RMSNorm &
          tính ở float32 &
          vượt 65504 $\rightarrow$ tràn $\rightarrow$ NaN cả mô hình \\
          \addlinespace
          GradScaler + cắt gradient &
          \texttt{unscale\_()} trước khi clip &
          clip gradient đã bị giãn, ngưỡng $1{,}0$ vô nghĩa \\
          \bottomrule
        \end{tabular}
      \end{block}
      \begin{beamercolorbox}[wd=\textwidth, rounded=true, sep=5pt]{block body alerted}
        \textcolor{MLIOTink}{\scriptsize T4 là kiến trúc Turing (CC 7.5),
        \textbf{không có bf16}. Chốt cứng \texttt{fp16} trong YAML.}
      \end{beamercolorbox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[shrink=10]{Kiểm chứng tính đúng đắn kiến trúc}
  \begin{columns}[T]
    ...
    \begin{column}{0.47\textwidth}
      ...
      \vspace{0.4em}
      \begin{beamercolorbox}[wd=\textwidth, rounded=true, sep=6pt]{block body alerted}
        \textcolor{MLIOTink}{\footnotesize\textbf{Kết quả cổng chặn:} loss cuối
        $< 0{,}05$ \xong{}, BLEU trên chính 50 câu đó \textbf{100{,}00} (yêu cầu
        $> 90$) \xong{}.
        \par\vspace{0.2em}
        Mô hình \textbf{không} học thuộc nổi 50 câu là kiến trúc còn sai — chưa qua
        bước này thì chưa được huấn luyện chính thức.}
      \end{beamercolorbox}
    \end{column}
  \end{columns}
\end{frame}
...
% ------------------------------------------------------------
%  SLIDE 12 – Chốt cấu hình bằng số đo
% ------------------------------------------------------------
\begin{frame}[shrink=15]{Chốt cấu hình bằng số đo, không lấy từ bài báo}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
      \begin{block}{Khảo sát trên Tesla T4 — fp16, 30 bước mỗi cấu hình}
        \scriptsize\bangchat
        \begin{tabular}{@{}rrrrrrr@{}}
          \toprule
          \textbf{Lớp} & \textbf{Head} & \texttt{d\_ff} & \textbf{Drop} &
          \textbf{Tham số} & \textbf{Giây/bước} & \textbf{VRAM} \\
          \midrule
          4 & 4 & 688  & 0{,}3 & 37.432.320 & 0{,}2728 & 6.959 \\
          4 & 8 & 688  & 0{,}3 & 37.432.320 & 0{,}2839 & 7.025 \\
          6 & 4 & 688  & 0{,}3 & 47.955.968 & 0{,}3572 & 8.016 \\
          \textbf{6} & \textbf{8} & \textbf{688} & \textbf{0{,}3} &
          \textbf{47.955.968} & \textbf{0{,}3710} & \textbf{8.112} \\
        ...
        \end{tabular}
      \end{block}
      \begin{block}{Ngân sách theo số token mỗi batch}
        \scriptsize\bangchat
        \begin{tabular}{@{}rrrrc@{}}
          \toprule
          \textbf{Token/batch} & \textbf{Giây/bước} & \textbf{Bước/epoch} &
          \textbf{Tổng giờ GPU} & \textbf{Khả thi} \\
          \midrule
          2048 & 0{,}8019 & 351 & 13{,}37 & CÓ \\
          \textbf{4096} & \textbf{1{,}5000} & \textbf{177} & \textbf{25{,}00} &
          \textbf{CÓ} \\
          8192 & --- & --- & --- & tràn bộ nhớ \\
          \bottomrule
        \end{tabular}
    ...
    \begin{column}{0.41\textwidth}
      \begin{block}{Kết luận rút ra từ bảng}
        \begin{itemize}
          \setlength{\itemsep}{0.2em}
          \item Chọn \textbf{6 lớp, 8 head, \texttt{d\_ff} 688, 4096 token/batch}
          \item 339 epoch tốn khoảng \textbf{25{,}00 giờ GPU}; ngân sách nhóm 30
            giờ/tuần $\rightarrow$ \textbf{khả thi}
          \item Mức 8192 token/batch \textbf{tràn bộ nhớ} T4, loại ngay
        \end{itemize}
      \end{block}
      \begin{beamercolorbox}[wd=\textwidth, rounded=true, sep=6pt]{block body alerted}
        \textcolor{MLIOTink}{\footnotesize\textbf{Nhanh nhất không đồng nghĩa với tốt
        nhất.} Cấu hình 4 lớp / 4 head nhanh hơn nhưng sức biểu diễn kém hơn. Con số
        cuối cùng phải chốt cùng với BLEU chứ không chỉ nhìn mỗi tốc độ.}
      \end{beamercolorbox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[shrink=15]{Huấn luyện theo epoch — một chu kỳ cập nhật}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{Thứ tự đúng của một bước optimizer}
        \scriptsize
        \texttt{for i in range(so\_buoc\_cong\_don):} \\
        \hspace{1em}\texttt{with autocast(float16):} \\
        \hspace{2em}\texttt{loss = tinh\_loss(...) / so\_buoc\_cong\_don} \\
        \hspace{1em}\texttt{scaler.scale(loss).backward()} \\
        \texttt{scaler.unscale\_(optimizer)} \\
        \texttt{clip\_grad\_norm\_(params, 1.0)} \\
        \texttt{scaler.step(optimizer); scaler.update()} \\
        \texttt{optimizer.zero\_grad(set\_to\_none=True)} \\
        \texttt{scheduler.step()}
        \par\vspace{0.3em}
        \textbf{Không gọi} \texttt{model.half()} — sẽ ép bảng góc quay của RoPE xuống
        fp16. Dùng \texttt{torch.autocast}.
      \end{block}
...
\begin{frame}[shrink=15]{Checkpoint chịu lỗi và đồng bộ Hugging Face Hub}
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      ...
      \end{block}
      \begin{block}{Bảo mật và ba cái bẫy đã gặp thật}
        \footnotesize
        \begin{itemize}
          \setlength{\itemsep}{0.15em}
          \item Token đọc theo thứ tự \textbf{Kaggle Secrets $\rightarrow$ biến môi
            trường \texttt{HF\_TOKEN} $\rightarrow$ báo lỗi rõ ràng}. Không viết token
            vào code, không commit.
          \item Smoke test \textbf{mặc định không đẩy} lên nhánh thật — lần trước một
            lượt smoke đã đè mất checkpoint thật.
          \item Luôn \textbf{liệt kê file trên repo} trước khi kết luận "Hub chưa có
            gì" — lần trước kết luận nhầm rồi train lại 3 tiếng.
        \end{itemize}
      \end{block}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=10]{Thí nghiệm giết phiên và phục hồi}
  \begin{columns}[T]
    ...
    \begin{column}{0.41\textwidth}
      ...
      \end{block}
      \begin{beamercolorbox}[wd=\textwidth, rounded=true, sep=6pt]{block body alerted}
        \textcolor{MLIOTink}{\footnotesize Thời gian lưu và đẩy checkpoint chiếm
    ...
    \begin{column}{0.4\textwidth}
      ...
      \begin{block}{Đọc con số này thế nào}
        ...
        \begin{itemize}
          ...
          \item Hướng xử lý là tăng dropout hoặc bật label smoothing, chính là thí
            nghiệm \textbf{A3}.
        \end{itemize}
      \end{block}
    \end{column}
    ...
    \begin{column}{0.48\textwidth}
      \begin{block}{Quy trình chấm điểm — hạ tầng đã sẵn sàng}
        ...
        \begin{itemize}
          ...
          \item Sinh câu: \textbf{Greedy} làm mốc, \textbf{Beam $= 4$} để so
          \item \textbf{KV cache} đã đo thật trên 40 câu tst2013: ra \textbf{đúng cùng
            một chuỗi token} mà nhanh gấp \textbf{5{,}5 lần} ($20{,}3 \rightarrow
            3{,}7$ giây)
          \item Hướng Anh $\rightarrow$ Việt, tập \textbf{tst2013}, \textbf{1.268} câu,
            chỉ dùng \textbf{một lần} ở bước cuối
        \end{itemize}
      \end{block}
      \begin{block}{Bảng điểm — đối chứng, 3.000 bước, 2 seed}
        \scriptsize\bangchat
        \begin{tabular}{@{}lcc@{}}
          \toprule
          \textbf{Tập} & \textbf{BLEU} & \textbf{chrF++} \\
          \midrule
          tst2012 (dev)  & 25{,}41 / 25{,}45 & 44{,}77 / 44{,}73 \\
          \textbf{tst2013 (test)} & \textbf{28{,}74 / 28{,}37} &
            \textbf{47{,}62 / 47{,}50} \\
          \bottomrule
        \end{tabular}
        \par\vspace{0.3em}
        Hai số là hai seed (42 / 1337). Sinh câu bằng greedy.
        \textbf{Vượt mốc $\ge 19$ của đề tài}, sát mốc "rất tốt" $\ge 29$ — mà mới
        chỉ \textbf{3.000 bước}, tức 17\% ngân sách của lượt huấn luyện chính.
      \end{block}
    \end{column}
    \begin{column}{0.48\textwidth}
      ...
      \end{block}
      \begin{beamercolorbox}[wd=\textwidth, rounded=true, sep=6pt]{block body alerted}
        \textcolor{MLIOTink}{\footnotesize\textbf{A1 — trả lời câu mentor hỏi.}
    ...
    \begin{column}{0.52\textwidth}
      \begin{block}{Đã làm được}
        ...
        \begin{itemize}
          ...
          \item \textbf{Kết quả dịch:} \textbf{28{,}74} BLEU và \textbf{47{,}62}
            chrF++ trên tst2013 (sacrebleu), chỉ với \textbf{3.000 bước} và
            \textbf{chỉ dùng dữ liệu trong ràng buộc} — vượt mốc $\ge 19$ của đề tài.
        \end{itemize}
      \end{block}
    \end{column}
    \begin{column}{0.44\textwidth}
      \begin{block}{Còn dang dở}
        ...
        \begin{itemize}
          ...
          \item Phân tích lỗi trên các câu dịch tệ — \chuaxong{}
      ...
      \begin{beamercolorbox}[wd=\textwidth, rounded=true, sep=7pt]{block body alerted}
        ...
        \textcolor{MLIOTink}{\footnotesize
        \begin{itemize}
          \setlength{\itemsep}{0.1em}
          \item Chạy đủ ablation 2 seed để trả lời dứt điểm RMSNorm vs LayerNorm
...
% ------------------------------------------------------------
\begin{frame}[shrink=10]{Tài liệu tham khảo (1/2)}
...
% ------------------------------------------------------------
\begin{frame}[shrink=10]{Tài liệu tham khảo (2/2)}
  \begin{thebibliography}{99}
    \setbeamertemplate{bibliography item}[text]
    \scriptsize
